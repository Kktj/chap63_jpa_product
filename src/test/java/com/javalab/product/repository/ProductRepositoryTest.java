...
public class ProductRepositoryTest {
	...
	//@Disabled
	@Commit	// 기본은 롤백
	public void testProductSaveBulder() {
	...
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(100); // 데이터베이스 확인
	 ...
	 * 
	 * [Fetch 전략 - Eager / Lazy]
	 * - Fetch 전략을 Lazy로 하게 되면 지연로딩이 되어 처음에는
	 * 	 상품 테이블만 조회하고 카테고리 테이블이 필요한 시점 즉,
	 * 	 product.toString()이 되는 시점에
	 * 	 카테고리 테이블에 조회해서 카테고리 관련 모든 정보를 조회한다.
	 * - Fetch 전략을 안주거나, Eager로 하게 되면 처음 부터 상품
	...
	public void testGetProductById() {
		...
		// 1. 상품 엔티티의 Category 멤버 변수에 지연로딩을 설정해놓게
		// 		되면 이 시점까지는 상품 테이블만 조회함.
	...
	public void testProductUpdate() {
		...
		Product product = new Product();
		product.setProductId(1);	// 실존하는 상품명으로
